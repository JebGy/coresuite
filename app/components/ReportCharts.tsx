...
export const ReportCharts: React.FC<ReportChartsProps> = ({
  ...
  const productosMasMovidos = React.useMemo(() => {
    const movimientosPorProducto = productos
      .map((producto) => {
  ...
  const valorInventarioPorAlmacen = React.useMemo(() => {
    const valorPorAlmacen = almacenes.map((almacen) => {
      const movs = movimientos.filter(
        (m) => m.almacenId === almacen.id && m.tipo === "entrada"
      );
      const valor = movs.reduce(
        ...
        >
          ...
          >
            ...
            <path
              ...
              strokeLinejoin="round"
              strokeWidth={2}
              d="M21 19l-5-5a2 2 0 00-2.828 0L7 19"
            />
          </svg>
          Exportar Imagen
        </button>
          ...
          onClick={() => {
            ...
            const stockPorProducto = productos.map((producto) => {
              ...
              return {
                ...
                Almacén:
                  almacenes.find((a) => a.id === producto.almacenId)?.nombre ||
                  "",
                Stock: stock,
              };
            });
          ...
          onClick={() => {
            ...
            productos.forEach((producto) => {
              almacenes.forEach((almacen) => {
                const movs = movimientos.filter(
                  (m) =>
                    m.productoId === producto.id && m.almacenId === almacen.id
                );
                let stock = 0;
                let valor = 0;
                let costoPromedio = 0;
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {almacenes.map((almacen) => {
                  const movs = movimientos.filter(
                    (m) => m.almacenId === almacen.id
                  );
                  const valor = movs.reduce((sum, m) => {
                    if (m.tipo === "entrada") {
                      return sum + m.cantidad * (m.precioUnitario ?? 0);
                    } else if (m.tipo === "salida") {
                      return sum - m.cantidad * (m.precioUnitario ?? 0);
