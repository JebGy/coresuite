...
function ProveedoresView() {
  ...
  const generateRegistrationLink = async (segmentoId?: number) => {
    setGeneratingLink(true);
    try {
      const response = await fetch('/api/proveedores/generate-token', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          expiresInHours: 72 // 3 días por defecto
        })
      });
      ...
      if (data.success) {
        ...
        setNotificacion({
          mensaje: 'Link de registro generado exitosamente',
          tipo: 'success'
        });
      } else {
        setNotificacion({
          mensaje: data.message || 'Error al generar el link',
          tipo: 'error'
        });
      }
    } catch (error) {
      ...
      setNotificacion({
        mensaje: 'Error de conexión al generar el link',
        tipo: 'error'
      });
    } finally {
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      setNotificacion({
        mensaje: 'Error al copiar el link',
        tipo: 'error'
      });
    }
  };
  ...
  return (
    <div className="space-y-6 col-span-full">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {generatingLink ? 'Generando...' : 'Generar Link de Registro'}
            </button>
            <button
      ...
      {/* Modal para mostrar el link de registro */}
      {showLinkModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-gray-800">Link de Registro Generado</h3>
              <button
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-2">
                Comparte este link con los proveedores para que puedan registrarse:
              </p>
